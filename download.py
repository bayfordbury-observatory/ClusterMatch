...
#Create post data payload (input coordinates, output format.. etc)
postData = {  '//source' : 'I/322', '//tables' : 'I/322A/out', '-out.max' : '10000',  '-out.form' : '| -Separated-Values',  '-oc.form' : 'dec',  '-c.eq' : 'J2000', '-c.r' : '++2', '-c.u' : 'arcmin', '-c.geom' : 'r', '-out.src' : 'I/322A/out', '-source' : 'I/322A/out', '-out.orig' : 'standard',  '-out' : 'RAJ2000, DEJ2000, Vmag, Bmag', 'Bmag' : '>0',  'Vmag' : '>0', 'RAJ2000' : str(RA)+'+/-0.5',  'DEJ2000' : str(DEC)+'+/-0.5','-bmark' : 'POST'}

#URL encode data
data = parse.urlencode(postData).encode()

#Create web request
req =  request.Request('https://vizier.u-strasbg.fr/viz-bin/asu-tsv', data=data)
